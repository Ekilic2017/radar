...
const Modal = ({detailId,close}) => {
  ...
  useEffect(()=>{
    //önceki uçusun detaylarını sıfırlar
    setDetail(null);
...
<span></span>
          ...
          <p><span>Kuyruk kodu:</span>
          <span>{d.aircraft.registration}</span>
          </p>
          {d.aircraft.images?(
            <img src={
              d.aircraft.images?.large ?d.aircraft.images.large[0].src:d.aircraft.images.thumbnails[0].src
            }alt="" />
          ):(<p>Fotograf bulunamadı</p>)}
          <p><span>Şirket:</span>
          ...
          <a href={d.airport?.destination?.website} target='_blank'>
            ...
            <p><span>Kalkış zamanı:</span>
            <span>{d.time?.scheduled?.departure>0?formatDate(d.time?.scheduled?.departure):"Bilinmiyor..."}</span>
            </p>
            <p><span>İniş zamanı:</span>
            <span>{d.time?.scheduled?.departure>0?formatDate(d.time?.scheduled?.arrival):"Bilinmiyor..."}</span>
            </p>
